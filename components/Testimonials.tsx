    ...
    return ( 
        <main className="px-[5%] py-[100px] grid grid-cols-1 md:grid-cols-2 gap-6">
            <section className="grid gap-4">
                <p className="text-[18px] text-gray-500 font-semibold special-font text-center md:text-left" data-aos="fade-down">Testimonials</p>
                ...
                <div className="flex items-center gap-2 justify-center md:justify-start" data-aos="slide-right">
                    <GoDotFill className={`text-2xl text-gray-700 ${currentIndex === 0 ? 'text-gray-700' : 'text-gray-500'} cursor-pointer`} onClick={() => handleDotClick(0)}/>
                    <GoDotFill className={`text-2xl text-gray-700 ${currentIndex === 1 ? 'text-gray-700' : 'text-gray-500'} cursor-pointer`}  onClick={() => handleDotClick(1)}/>
                    <GoDotFill className={`text-2xl text-gray-700 ${currentIndex === 2 ? 'text-gray-700' : 'text-gray-500'} cursor-pointer`}  onClick={() => handleDotClick(2)}/>
                </div>
            </section>
            ...
            <section className="flex items-center justify-center gap-4 mt-10 md:mt-0 special-font">
                <div className="relative">
                    <div className="p-4 rounded-xl shadow-lg bg-white w-[85%] md:w-[70%] grid gap-3 relative z-10 " data-aos="fade-down">
                        ...
                        <p className="text-[13px] md:text-[15px] text-gray-500">{testimonialsData[currentIndex].review}</p>

                        <h1 className="text-[16px] md:text-[18px] text-gray-500 font-semibold">{testimonialsData[currentIndex].name}</h1>
                        <p className="text-[12px] md:text-[14px] text-gray-500">{testimonialsData[currentIndex].company}</p>
                    </div>
                    ...
                    <div className="p-4 rounded-xl shadow-sm border-[#F7F7F7] border bg-white w-[85%] md:w-[70%] grid gap-3 absolute left-[50px] top-[70px]" data-aos="fade-down">
                        ...
                        <p className="text-[13px] md:text-[15px] text-gray-500">{testimonialsData[(currentIndex + 1) % testimonialsData.length].review}</p>

                        <h1 className="text-[16px] md:text-[18px] text-gray-500 font-semibold">{testimonialsData[(currentIndex + 1) % testimonialsData.length].name}</h1>
                        <p className="text-[12px] md:text-[14px] text-gray-500">{testimonialsData[(currentIndex + 1) % testimonialsData.length].company}</p>
                    </div>
                </div>

                <div className="flex flex-col gap-4" data-aos="slide-right">
                    <Image src="/icons/up.svg" alt="up-arrow" width={40} height={40}
                    onClick={handleArrowUpClick}
                    />

                    <Image src="/icons/down.svg" alt="down-arrow" width={40} height={40}
